package ru.job4j.dreamjob.controller;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.mockito.ArgumentCaptor;
import org.springframework.mock.web.MockMultipartFile;
import org.springframework.ui.ConcurrentModel;
import org.springframework.web.multipart.MultipartFile;
import ru.job4j.dreamjob.dto.FileDto;
import ru.job4j.dreamjob.model.City;
import ru.job4j.dreamjob.model.Vacancy;
import ru.job4j.dreamjob.service.CityService;
import ru.job4j.dreamjob.service.VacancyService;

import java.io.IOException;
import java.util.List;
import java.util.Optional;

import static org.assertj.core.api.Assertions.*;

import static java.time.LocalDateTime.now;
import static org.mockito.Mockito.*;

class VacancyControllerTest {

    private VacancyService vacancyService;

    private CityService cityService;

    private VacancyController vacancyController;

    /**
     * Данное поле описывает загруженный
     * по запросу файл.
     *
     * В Spring для тестирования подобных
     * файлов имеется специальный класс
     * {@link MockMultipartFile}.
     */
    private MultipartFile testFile;

    @BeforeEach
    public void initServices() {
        vacancyService = mock(VacancyService.class);
        cityService = mock(CityService.class);
        vacancyController = new VacancyController(vacancyService, cityService);
        testFile = new MockMultipartFile("testFile.img", new byte[] {1, 2, 3});
    }

    /**
     * Во всех тестах для создания Model мы
     * используем готовый класс, который использует
     * Spring - это {@link ConcurrentModel}.
     * Это реализация Model на основе ConcurrentHashMap.
     */
    @Test
    public void whenRequestVacancyListPageThenGetPageWithVacancies() {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var vacancy2 = new Vacancy(2, "test2", "desc2", now(), false, 3, 4);
        var expectedVacancies = List.of(vacancy1, vacancy2);
        when(vacancyService.findAll()).thenReturn(expectedVacancies);

        var model = new ConcurrentModel();
        var view = vacancyController.getAll(model);
        var actualVacancies = model.getAttribute("vacancies");

        assertThat(view).isEqualTo("vacancies/list");
        assertThat(actualVacancies).isEqualTo(expectedVacancies);
    }

    @Test
    public void whenRequestVacancyCreationPageThenGetPageWithCities() {
        var city1 = new City(1, "Москва");
        var city2 = new City(2, "Санкт-Петербург");
        var expectedCities = List.of(city1, city2);
        when(cityService.findAll()).thenReturn(expectedCities);

        var model = new ConcurrentModel();
        var view = vacancyController.getCreationPage(model);
        var actualVacancies = model.getAttribute("cities");

        assertThat(view).isEqualTo("vacancies/create");
        assertThat(actualVacancies).isEqualTo(expectedCities);
    }

    /**
     * В данном тесте используется класс {@link ArgumentCaptor}.
     * Этот класс позволяет "захватить" аргумент, который
     * передается в метод. Он удобен при тестировании
     * методов аргументы, которых вычисляются, как,
     * например, при создании FileDto.
     *
     * Мы не можем получить их как возвращаемые значения!
     *
     * В месте вызова метода нам нужно вызывать метод
     * {@link ArgumentCaptor#capture()}.
     * Для получения переловленного значения нужно
     * вызвать метод {@link ArgumentCaptor#getValue()}.
     */
    @Test
    public void whenPostVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.save(vacancyArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(vacancy);

        var model = new ConcurrentModel();
        var view = vacancyController.create(vacancy, testFile, model);
        var actualVacancy = vacancyArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(actualVacancy).isEqualTo(vacancy);
        assertThat(fileDto).usingRecursiveComparison().isEqualTo(actualFileDto);

    }


    /**
     * В данном тесте наше исключение
     * ловится в методе
     * {@link VacancyService#save},
     * поэтому мы здесь объявляем
     * RuntimeException.
     *
     * Мы ожидаем, что метод поймает
     * исключение и мы прочитаем
     * текст исключения.
     */
    @Test
    public void whenSomeExceptionThrownThenGetErrorPageWithMessage() {
        var expectedException = new RuntimeException("Failed to write file");
        when(vacancyService.save(any(), any())).thenThrow(expectedException);

        var model = new ConcurrentModel();
        var view = vacancyController.create(new Vacancy(), testFile, model);
        var actualExceptionMessage = model.getAttribute("message");

        assertThat(view).isEqualTo("errors/404");
        assertThat(actualExceptionMessage).isEqualTo(expectedException.getMessage());
    }

    @Test
    public void whenRequestVacancyByIdThenGetConcreteVacancyPage() {
        var vacancy = new Vacancy(1, "Vaca", "desc", now(), true, 1, 2);
        var expectedVacancy = vacancyService.findById(anyInt());
        when(expectedVacancy).thenReturn(Optional.of(vacancy));

        var model = new ConcurrentModel();
        var view = vacancyController.getByID(model, anyInt());
        var actualVacancies = model.getAttribute("vacancy");

        assertThat(view).isEqualTo("vacancies/one");
        assertThat(actualVacancies).isEqualTo(vacancy);
    }

    @Test
    public void whenRequestVacancyByWrongIdThenGetErrorPageWithMessage() {
        when(vacancyService.findById(1)).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = vacancyController.getByID(model, 1);
        var errorMessage = model.getAttribute("message");

        assertThat(view).isEqualTo("errors/404");
        assertThat(errorMessage).isEqualTo("Вакансия с указанным идентификатором не найдена");
    }

    @Test
    public void whenRequestVacancyUpdateThenGetUpdatedVacancy() throws IOException {
        var vacancy = new Vacancy(1, "Vaca", "desc", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(vacancyArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = vacancyController.update(vacancy, testFile, model);
        var actualVacancy = vacancyArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(actualVacancy).isEqualTo(vacancy);
        assertThat(actualFileDto).usingRecursiveComparison().isEqualTo(fileDto);
    }

    @Test
    public void whenRequestVacancyUpdateByWrongIdThenGetErrorPage() throws IOException {
        var expectedException = new RuntimeException("Вакансия с указанным идентификатором не найдена");
        var vacancy = new Vacancy(1, "Vaca", "desc", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        when(vacancyService.update(vacancy, fileDto)).thenThrow(expectedException);

        var model = new ConcurrentModel();
        var view = vacancyController.update(vacancy, testFile, model);
        var actualErrorMessage = model.getAttribute("message");

        assertThat(view).isEqualTo("errors/404");
        assertThat(actualErrorMessage).isEqualTo(expectedException.getMessage());
    }

    @Test
    public void whenRequestVacancyDeleteThenRedirectToVacanciesPage() throws IOException {
        var vacancy = new Vacancy(1, "Vaca1", "desc", now(), true, 1, 1);
        when(vacancyService.deleteById(vacancy.getId())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, vacancy.getId());
        var actualVacancies = model.getAttribute("vacancies");

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(actualVacancies).isNull();
    }

    @Test
    public void whenRequestUnexistedVacancyDeleteThenGetErrorPage() {
        when(vacancyService.deleteById(anyInt())).thenReturn(false);

        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, anyInt());
        var actualErrorPage = model.getAttribute("message");

        assertThat(view).isEqualTo("errors/404");
        assertThat(actualErrorPage).isEqualTo("Вакансия с указанным идентификатором не найдена");
    }
}